...
export default function FilesSidebar({ files, selectedFileId, onFileClick, statements }: FilesSidebarProps) {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} min-h-screen bg-gray-800/90 backdrop-blur-sm border-r border-gray-700/50 flex flex-col shadow-lg transition-all duration-300`}>
      ...
      <div className="p-4 border-b border-gray-700/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {!isCollapsed && (
              <div>
                <h2 className="text-sm font-bold text-white">Files</h2>
                <p className="text-xs text-gray-400">Manage documents</p>
              </div>
            )}
          </div>
          <button
          ...
          >
            {isCollapsed ? (
              <RiMenuUnfoldLine className="w-4 h-4 text-gray-300" />
            ) : (
              <RiMenuFoldLine className="w-4 h-4 text-gray-300" />
            )}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-1">
          ...
          >
            <div className={`w-2 h-2 rounded-full ${
              selectedFileId === 'all' ? 'bg-white' : 'bg-gray-500'
            }`}></div>
            {!isCollapsed && (
              <div>
          ...
          <div className="mt-4">
            {!isCollapsed && (
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 px-3">Banks</h3>
            )}
            <div className="space-y-1">
              {files.map((bank) => (
                <button
                ...
                >
                  <div className={`w-2 h-2 rounded-full ${
                    selectedFileId === bank.id ? 'bg-white' : 'bg-gray-500'
                  }`}></div>
                  {!isCollapsed && (
      ...
      {!isCollapsed && (
        <div className="p-3 border-t border-gray-700/50">
          <div className="text-center">
            ...
            <div className="text-lg font-bold text-white">{statements.length}</div>
          </div>
        </div>
      )}
    </aside>
